...
export function AnnotationOverlay({
	...
	const renderArrow = () => {
		...
		const color = annotation.figureData?.color || "#e11d2e";
	...
	return (
		<Rnd
			...
			className={cn(
				...
				isSelected && "ring-2 ring-[#e11d2e] ring-offset-2 ring-offset-transparent",
			)}
			style={{
				zIndex: isSelectedBoost ? zIndex + 1000 : zIndex, // Boost selected annotation to ensure it's on top
				pointerEvents: "auto",
				border: isSelected ? "2px solid rgba(225, 29, 46, 0.8)" : "none",
				backgroundColor: isSelected ? "rgba(225, 29, 46, 0.1)" : "transparent",
				boxShadow: isSelected ? "0 0 0 1px rgba(225, 29, 46, 0.35)" : "none",
			}}
			...
			resizeHandleStyles={{
				topLeft: {
					...
					border: isSelected ? "2px solid #e11d2e" : "none",
					borderRadius: "50%",
					left: "-6px",
					top: "-6px",
					cursor: "nwse-resize",
