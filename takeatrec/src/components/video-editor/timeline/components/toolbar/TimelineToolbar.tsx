	...
	return (
		<div className="flex items-center gap-2 px-4 py-2 border-b border-foreground/10 bg-editor-panel">
			<div className="flex items-center gap-1">
				<Button onClick={onAddZoom} variant="ghost" size="icon" className="h-7 w-7 text-muted-foreground hover:text-[#e11d2e] hover:bg-[#e11d2e]/10 transition-all" title={addZoomLabel} aria-label={addZoomLabel}>
					<ZoomIn className="w-4 h-4" />
				</Button>
				<Button onClick={onSuggestZooms} variant="ghost" size="icon" className="h-7 w-7 text-muted-foreground hover:text-[#e11d2e] hover:bg-[#e11d2e]/10 transition-all" title={suggestZoomsLabel} aria-label={suggestZoomsLabel}>
			...
			<div className="flex items-center gap-2">
					...
					<DropdownMenuContent align="end" className="bg-editor-surface-alt border-foreground/10">
						{ASPECT_RATIOS.map((ratio) => (
							<DropdownMenuItem key={ratio} onClick={() => onAspectRatioChange?.(ratio)} className="text-muted-foreground hover:text-foreground hover:bg-foreground/10 cursor-pointer flex items-center justify-between gap-3">
								...
								{aspectRatio === ratio && <Check className="w-3 h-3 text-[#e11d2e]" />}
							</DropdownMenuItem>
						))}
						...
						<div className="px-2 py-1.5 flex items-center gap-2 text-muted-foreground">
							...
							<input type="text" inputMode="numeric" value={customAspectWidth} onChange={(event) => onCustomAspectWidthChange(event.target.value.replace(/\D/g, ""))} onKeyDown={onCustomAspectRatioKeyDown} className="w-12 h-7 rounded border border-foreground/10 bg-foreground/5 px-1.5 text-sm text-foreground focus:outline-none focus:ring-1 focus:ring-[#e11d2e]" aria-label="Custom aspect width" />
							<span className="text-muted-foreground/70">:</span>
							<input type="text" inputMode="numeric" value={customAspectHeight} onChange={(event) => onCustomAspectHeightChange(event.target.value.replace(/\D/g, ""))} onKeyDown={onCustomAspectRatioKeyDown} className="w-12 h-7 rounded border border-foreground/10 bg-foreground/5 px-1.5 text-sm text-foreground focus:outline-none focus:ring-1 focus:ring-[#e11d2e]" aria-label="Custom aspect height" />
							...
							{isCustomAspectRatio(aspectRatio) && <Check className="w-3 h-3 text-[#e11d2e] ml-auto" />}
						</div>
					</DropdownMenuContent>
				</DropdownMenu>
				...
				>
					...
					{isCropped ? <span className="h-1.5 w-1.5 rounded-full bg-[#e11d2e]" /> : null}
				</Button>
			</div>
			<div className="flex-1" />
			<div className="flex items-center gap-4 text-[10px] text-muted-foreground/70 font-medium">
				<span className="flex items-center gap-1.5">
					<kbd className="px-1.5 py-0.5 bg-foreground/5 border border-foreground/10 rounded text-[#e11d2e] font-sans">Side Scroll</kbd>
					<span>Pan</span>
				</span>
				<span className="flex items-center gap-1.5">
					<kbd className="px-1.5 py-0.5 bg-foreground/5 border border-foreground/10 rounded text-[#e11d2e] font-sans">{scrollLabels.pan}</kbd>
					<span>Pan</span>
				</span>
				<span className="flex items-center gap-1.5">
					<kbd className="px-1.5 py-0.5 bg-foreground/5 border border-foreground/10 rounded text-[#e11d2e] font-sans">{scrollLabels.zoom}</kbd>
					<span>Zoom</span>
				</span>
			</div>
		</div>
