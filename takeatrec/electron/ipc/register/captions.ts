	...
	ipcMain.handle("open-video-file-picker", async (_, options?: OpenVideoFilePickerOptions) => {
		try {
			...
			const result = await dialog.showOpenDialog({
				title: includeProjects ? "Import Media or Takeat Rec Project" : "Select Video File",
				defaultPath: recordingsDir,
				filters: [
					...(includeProjects
						? [
								{
									name: "Media or Takeat Rec Projects",
									extensions: [
					...
					...(includeProjects
						? [{ name: "Takeat Rec Projects", extensions: PROJECT_FILE_EXTENSIONS }]
						: []),
