...
// Botão "voltar" do editor: fecha a janela do editor (respeitando o aviso
// de alterações não salvas já existente) e, só depois de fechar de fato,
// restaura a janela de gravação (que ficou apenas oculta, não destruída).
ipcMain.on("editor-back-to-recording", (event) => {
...
// watchers). Senão o servidor fica fantasma e REABRE a janela sozinho a
// cada rebuild, muitas vezes num estado zumbi sem clique nem arrasto.
app.on("quit", () => {
	if (process.env.VITE_DEV_SERVER_URL && process.ppid) {
		try {
			const { spawn: spawnKill } = require("node:child_process") as typeof import("node:child_process");
			spawnKill("taskkill", ["/PID", String(process.ppid), "/T", "/F"], {
				windowsHide: true,
				stdio: "ignore",
			}).unref();
		} catch {
...
// Botão "voltar" do Takeat Rec: reabre o launcher Takeat Studio e encerra.
// O launcher (../app) tem trava de instância única, então se já estiver
// aberto ele só é trazido de volta à frente.
ipcMain.on("open-takeat-studio", () => {
	const { spawn } = require("node:child_process") as typeof import("node:child_process");
	const { existsSync } = require("node:fs") as typeof import("node:fs");

	// Instalado: o Rec mora em <Studio>\resources\takeatrec\, então o exe do
	// Studio está três níveis acima. O Studio tem trava de instância única,
	// logo se já estiver aberto ele só volta pra frente.
	if (app.isPackaged) {
		const studioExe = path.resolve(process.resourcesPath, "..", "..", "..", "Takeat Studio.exe");
		if (existsSync(studioExe)) {
			spawn(studioExe, [], { detached: true, stdio: "ignore" }).unref();
			setTimeout(() => app.quit(), 400);
			return;
		}
	}

	const studioDir = path.resolve(process.env.APP_ROOT ?? process.cwd(), "..", "app");
	spawn(
		"powershell.exe",
		[
			"-NoProfile",
			...
			"Hidden",
			"-Command",
			`Start-Process -FilePath 'cmd.exe' -ArgumentList '/c','npm','start' -WorkingDirectory '${studioDir}' -WindowStyle Hidden`,
		],
...
function getUpdateNotificationTitle(payload: UpdateToastPayload) {
	switch (payload.phase) {
		case "available":
			return `Takeat Rec ${payload.version} is available`;
		case "downloading":
			return `Downloading Takeat Rec ${payload.version}`;
		case "ready":
			return `Takeat Rec ${payload.version} is ready`;
		case "error":
			return `Takeat Rec ${payload.version} needs attention`;
	}
}
...
function getUpdateNotificationBody(payload: UpdateToastPayload) {
	switch (payload.phase) {
		case "available":
			return "Click to install the update and restart Takeat Rec.";
		case "downloading":
			return "Takeat Rec is downloading the update and will restart when it is ready.";
		case "ready":
...
function updateTrayMenu(recording: boolean = false) {
	...
	const trayToolTip = recording ? `Recording: ${selectedSourceName}` : "Takeat Rec";
...
app.whenReady().then(async () => {
	...
	// Takeat Rec does not use WebHID, Web Serial, or WebUSB. Do not grant devices by default.
	...
	createWindow();
	// Atualização automática desligada: o updater aponta pro repositório
	// original do Recordly, então no Takeat Rec ele só produziria erro 404 e
	// avisos falsos de atualização. Distribuição é por instalador próprio.
	if (process.env.TAKEATREC_ENABLE_AUTOUPDATE === "1") {
